...
class PythonScriptWrapper(object):
    ...
    def upload_results(self, bq):
        ...
        print("ckck outputs_tag",outputs_tag)
        nonimage_tag = outputs_tag.find("./*[@name='NonImage']")
        print("ckck nonimage_tag", nonimage_tag.tag, nonimage_tag.attrib)
        ...
        for resource in (nonimage_tag.findall(".//*[@type]") + outputs_tag.findall("./*[@type='image']")): 
            ...
            print("NonImage type output with type %s" % resource.attrib['type'])

            log.info("***** resource.attrib %s '%s'..." % (resource.attrib['name'], resource.attrib['type']))

            resource_name = resource.attrib['name']
            resource_type = "resource" # resource.attrib['type'] 
...
#        print(inputs_tag)
    ...
    def upload_service(self, bq, filename, data_type='image'):
        ...
        log.info('ckckck Up Mex: %s' % (mex_id))
