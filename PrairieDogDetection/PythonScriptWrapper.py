...
from bqapi.util import fetch_blob, fetch_dataset
...
class PythonScriptWrapper(object):
    ...
    def fetch_input_resources(self, bq, inputs_dir_path): #TODO Not hardcoded resource_url
        ...
        print('connor1', inputs_tag)
        for input_resource in inputs_tag.findall("./*[@type]"):
            
            # if input_resource.attrib['type'] != 'resource':
            #     print(input_resource.tag,' between ' ,input_resource.attrib)
            #     log.info("***** Adding parameter named %s" % input_resource.attrib['name'])
            #     try:
            #         parameters_dict[input_resource.attrib['name']] = input_resource.attrib
            #     except KeyError:
            #         log.info("***** Parameter %s has no value" % input_resource.attrib['name'])
            #     continue

            if input_resource.attrib['type'] not in {'resource', 'string', 'boolean'}:
                continue
            ...
            if input_resource.attrib['type'] in {'string', 'boolean'}:
                parameters_dict[input_name] = getattr(self.options, input_name)
                continue


            resource_obj = bq.load(getattr(self.options, input_name))
            print('obj resource', resource_obj)
            ...
            if input_resource.attrib['type'] in {'string', 'boolean'}:
                parameters_dict[input_name] = resource_obj
                continue

            if input_name == 'Input Image':
        ...
        return input_path_dict, uri, parameters_dict
    ...
    def run(self):
        ...
        try:
            ...
            input_path_dict, uri, params = self.fetch_input_resources(bq, inputs_dir_path)
        ...
        try:
            ...
            self.output_data_path_dict = run_module(bq, input_path_dict, outputs_dir_path, uri, params) 
        except (Exception, ScriptError) as e:
