...
class PythonScriptWrapper(object):
	...
	"""
        ...
        # predictor_path = os.path.join(kw.get('stagingPath', 'INPUTS'), self.getstrtime()+'-'+image.name + '.txt')
        #
        ...
        nodes_uniq = self.options.nodes_url.split('/')[-1]
        nodes_url = bq.service_url('blob_service', path=nodes_uniq)
        nodes_path = os.path.join(kw.get('stagingPath', '/work'), 'nodes.txt')
        nodes_path = bq.fetchblob(nodes_url, path=nodes_path)

        nodetype_uniq = self.options.nodetype_url.split('/')[-1]
        nodetype_url = bq.service_url('blob_service', path=nodetype_uniq)
        nodetype_path = os.path.join(kw.get('stagingPath', '/work'), 'nodetype.txt')
        nodetype_path = bq.fetchblob(nodetype_url, path=nodetype_path)

        triangles_uniq = self.options.triangles_url.split('/')[-1]
        triangles_url = bq.service_url('blob_service', path=triangles_uniq)
        triangles_path = os.path.join(kw.get('stagingPath', '/work'), 'triangles.txt')
        triangles_path = bq.fetchblob(triangles_url, path=triangles_path)

        facelabels_uniq = self.options.facelabels_url.split('/')[-1]
        facelabels_url = bq.service_url('blob_service', path=facelabels_uniq)
        facelabels_path = os.path.join(kw.get('stagingPath', '/work'), 'facelabels.txt')
        facelabels_path = bq.fetchblob(facelabels_url, path=facelabels_path)
        ...
        #    f.write(ip2.fetch())
        # log.info('Executing Meshing')
        # log.info('Completed Meshing')
        return
    ...
    def uploadimgservice(self, bq, files):
        ...
        resource = etree.Element(
            'image', name='ModuleExecutions/xtalsmesh/'+filename)
    ...
    def run(self):
        ...
        # call scripts
        try:
            bq.update_mex('Loading INPUTS...')
            self.preprocess(bq)
            bq.update_mex('Applying Smoothing Function')
            subprocess.call(['python3', 'Xtal_Smoother.py'])
            bq.update_mex('Applying Meshing Function')
            subprocess.call(['python3', 'Xtal_Mesher.py', '0.01'])
        ...
        try:
            ...
            bq.update_mex('Uploading Mesh')
            self.meshvtk = self.uploadimgservice(bq, ['/work/XtalMesh.vtk'])
            self.meshinp = self.uploadimgservice(bq, ['/work/XtalMesh.inp'])
        ...
        log.info('Completed the workflow: %s' % (self.meshvtk.get('value')))
        out_imgxml = """<tag name="Segmentation" type="file" value="%s">
			<template>
		          <tag name="label" value="Meshed VTK Ouput" />
			</template>
		      </tag>""" % (str(self.meshvtk.get('value')))
