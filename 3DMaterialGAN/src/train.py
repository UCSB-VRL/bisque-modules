...
def initialize_generator(args):
    ...
    print('Loading generator from {}'.format(args['test_path']))
    g_checkpoint = torch.load(args['test_path'], map_location=torch.device("cpu") )
    g_model.load_state_dict(dict([(n, p) for n, p in g_checkpoint['model_state_dict'].items()]), strict=False)
...
def initialize_model(args, d_path, g_path):
    ...
    if d_path is not None and os.path.isfile(d_path) and g_path is not None and os.path.isfile(g_path):
        ...
        d_checkpoint = torch.load(d_path, map_location=torch.device("cpu") )
        g_checkpoint = torch.load(g_path, map_location=torch.device("cpu") )
